...
def cw_about_origin(point):
    """Rotate a single point 90 degrees in the SHAPES (y-up) frame.

    The SHAPES table was generated by repeatedly applying this to each piece's
    spawn state about the origin, so table rotation index N must equal N
    applications of it followed by a translation back to the origin.
    """
    x, y = point
    return (-y, x)
...
def test_line_clear_and_scoring():
    ...
    for y in range(18, 22):
        for x in range(9):
            g2.board.grid[y][x] = 'L'
    # Rotation 1 puts the four cells in a single column at board offset x+0, and
    # the cells span board rows anchor_y-1 .. anchor_y+2.
    g2.current = E.Piece('I', 9, 19, rotation=1)
    ...
    for y in range(18, 22):
        for x in range(9):
            g4.board.grid[y][x] = 'L'
    g4.current = E.Piece('I', 9, 19, rotation=1)
    g4.hard_drop()
